...
def getAllNodeAndIOname(model):
    for i in range(len(model.graph.node)):
        ...
        if i==246:
            print("index:{}, nodename:{}, input_name:{}, output_name:{}".format(i, Node, input_name, output_name))
    print(len(model.graph.node))

def getInputAndOutputNode(model):
    input = [params_input.name for params_input in model.graph.input]
    output = [params_output.name for params_output in model.graph.output]
    print("input:", input)
    print("output:", output)
...
if __name__ == "__main__":
    from utils.utils import ly_create_backbone
    ly_create_backbone('./weights/last.pt')

    # model = loadOnnxModel("./weights/export.onnx")
    # getAllNodeAndIOname(model)
    # getInputAndOutputNode(model)
    # getInputTensorValueInfo(['661', '684', '684', '683'], model)
