...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None):
    ...
    if ratio_pad is None:  # calculate from img0_shape
        ...
        pad = (img1_shape[1] - img0_shape[1] * gain) / 2, (img1_shape[0] - img0_shape[0] * gain) / 2  # wh paddin
    else:
        gain = ratio_pad[0][0]
        pad = ratio_pad[1]
...
def ly_create_backbone(f='weights/last.pt'):  # from utils.utils import *; create_backbone()
    # create a backbone from a *.pt file
    x = torch.load(f, map_location=torch.device('cpu'))
    model = torch.load(f, map_location='cpu')['model']
