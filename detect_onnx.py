...
def detect(save_txt=False, save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if img.ndimension() == 3:
            img = img.unsqueeze(0)
        # pred = model(img)
        # scores, boxes = torch.onnx._export(model, img, 'weights/export.onnx', verbose=False, opset_version=11)
        # pred = torch.cat([boxes, scores], dim=1).unsqueeze(0)
        # np.testing.assert_almost_equal(pred.data.cpu().numpy(), onnx_out.data.cpu().numpy(), decimal=3)
        pred = torch.onnx._export(model, img, 'weights/export.onnx', verbose=False, opset_version=10)[0]
        import onnx
        model = onnx.load('weights/export.onnx')  # Load the ONNX model
        onnx.checker.check_model(model)  # Check that the IR is well formed
        print(onnx.helper.printable_graph(model.graph))  # Print a human readable representation of the graph
